export default class ArticlesList {
  constructor(props) {
    ...
    this._articles = [];
    this._liked = [];
    this._articlesQuantity = articlesQuantity;
  }
  ...
  getArticlesQuantity(allArticles = false) {
    if (allArticles) {
      return this._articles;
    }

    const from = this._currentArticleIndex;
    const to = Math.min(this._currentArticleIndex + this._articlesQuantity, this._articles.length);
    if (from >= to) {
      return [];
    }
